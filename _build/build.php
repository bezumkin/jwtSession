...
class jwtSessionPackage
{
    ...
    /**
     * Removes unneeded files from composer
     */
    public function cleanVendor()
    {
            ...
            if ($dirs = @scandir($base)) {
                foreach ($dirs as $dir) {
                    if (!in_array($dir, ['src', 'vendor', '.', '..'])) {
                        ...
                        if (is_dir($path)) {
                            $manager->deleteTree($path, ['deleteTop' => true, 'extensions' => '']);
                        } else {
                            unlink($path);
                        }
                    }
...
$install = new jwtSessionPackage(MODX_CORE_PATH, $config);
